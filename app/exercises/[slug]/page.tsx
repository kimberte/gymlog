...
import { EXERCISE_DATABASE, getExercise } from "../../lib/exerciseDatabase";

type Props = { params: Promise<{ slug: string }> };

export function generateStaticParams() {
  return EXERCISE_DATABASE.map(x => ({ slug: x.slug }));
}
...
export default async function ExerciseDetailPage({ params }: Props) {
  ...
  const related = exercise.alternatives.map(slug => getExercise(slug)).filter(Boolean).slice(0,3);
  return <main className="exercise-detail-page">
    ...
    <article className="exercise-detail">
      ...
      <div className="exercise-visual">Gym Log exercise illustration coming in the next media phase.<br/>For now, the database focuses on accurate, original written guidance.</div>
