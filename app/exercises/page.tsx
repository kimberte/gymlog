...
export default function ExercisesPage() {
  ...
  return <main className="exercise-page">
    ...
    <section className="exercise-hero"><div className="exercise-kicker">THE GYM LOG EXERCISE DATABASE</div><h1>{pickerMode ? "Add an exercise to your workout." : "Learn an exercise. Then track it."}</h1><p>{pickerMode ? "Search the exercise library and tap any movement to add its exact name to your notebook. You can keep your workout simple while still connecting it to the Gym Log exercise library." : "Explore the Gym Log exercise database with practical instructions, muscles worked, equipment, movement patterns, common mistakes, tips and useful alternatives. The library is being built from original Gym Log content."}</p>{!pickerMode ? <div className="exercise-hero-links"><Link href="/">Open Workout Log →</Link><Link href="/workout-programs" className="secondary">Browse Programs →</Link></div> : null}</section>
    {pickerMode ? <div className="exercise-picker-banner"><div><strong>Picker mode:</strong> tap an exercise below to add it to the workout notebook.</div></div> : null}
    <section className="exercise-library" aria-label="Exercise database">
      ...
      {results.length ? <div className="exercise-grid">{results.map(x => pickerMode ? <button type="button" className="exercise-card exercise-picker-button" key={x.slug} onClick={() => selectExercise({ name: x.name, slug: x.slug })}><div className="exercise-card-top"><span>{x.category}</span><span>{x.difficulty}</span></div><h2>{x.name}</h2><p>{x.description}</p><div className="exercise-tags"><span className="exercise-tag">{x.movementPattern}</span>{x.primaryMuscles.slice(0,2).map(m => <span className="exercise-tag" key={m}>{m}</span>)}</div><div className="exercise-card-action">Add to workout →</div></button> : <Link href={`/exercises/${x.slug}`} className="exercise-card" key={x.slug}><div className="exercise-card-top"><span>{x.category}</span><span>{x.difficulty}</span></div><h2>{x.name}</h2><p>{x.description}</p><div className="exercise-tags"><span className="exercise-tag">{x.movementPattern}</span>{x.primaryMuscles.slice(0,2).map(m => <span className="exercise-tag" key={m}>{m}</span>)}</div><div className="exercise-card-action">View exercise →</div></Link>)}</div> : <div className="exercise-empty">No exercises match those filters. Try a broader search.</div>}
    </section>
