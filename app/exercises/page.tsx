...
export default function ExercisesPage() {
  ...
  useEffect(() => {
    setPickerMode(new URLSearchParams(window.location.search).get("picker") === "1");
  }, []);

  const equipmentOptions = useMemo(() => ["All", ...Array.from(new Set(EXERCISE_CATALOG.flatMap(x => x.equipment))).sort()], []);
  const results = useMemo(() => {
    const q = query.trim().toLowerCase();
    return EXERCISE_CATALOG.filter(x => {
  ...
  function selectExercise(exercise: { name: string; slug: string }) {
    window.parent.postMessage(
      { type: "gym-log-exercise-selected", name: exercise.name, slug: exercise.slug },
  ...
  return <main className="exercise-page">
    ...
    <section className="exercise-library" aria-label="Exercise database">
      ...
      <div className="exercise-results">Showing {results.length} of {EXERCISE_CATALOG.length} exercises</div>
      {results.length ? <div className="exercise-grid">{results.map(x => pickerMode ? <article className="exercise-card" key={x.slug}><div className="exercise-card-top"><span>{x.category}</span><span>{x.difficulty}</span></div><h2>{x.name}</h2><p>{x.description}</p><div className="exercise-tags"><span className="exercise-tag">{x.movementPattern}</span>{x.primaryMuscles.slice(0,2).map(m => <span className="exercise-tag" key={m}>{m}</span>)}</div><div className="exercise-picker-actions"><Link className="exercise-view-button" href={`/exercises/${x.slug}?picker=1`}>View exercise</Link><button type="button" className="exercise-add-button" onClick={() => selectExercise({ name: x.name, slug: x.slug })}>+ Add</button></div></article> : <Link href={`/exercises/${x.slug}`} className="exercise-card" key={x.slug}><div className="exercise-card-top"><span>{x.category}</span><span>{x.difficulty}</span></div><h2>{x.name}</h2><p>{x.description}</p><div className="exercise-tags"><span className="exercise-tag">{x.movementPattern}</span>{x.primaryMuscles.slice(0,2).map(m => <span className="exercise-tag" key={m}>{m}</span>)}</div><div className="exercise-card-action">View exercise →</div></Link>)}</div> : <div className="exercise-empty">No exercises match those filters. Try a broader search.</div>}
    </section>
    {!pickerMode ? <section className="exercise-note"><div className="exercise-note-box"><strong>Building this library:</strong> Gym Log is starting with a focused set of high-value movements and will expand into more variations, machines, conditioning movements and original illustrations. Exercise pages are educational resources, not medical advice; use appropriate loads and seek qualified coaching when you need individualized instruction.</div></section> : null}
  </main>;
}
