...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const staticPages: MetadataRoute.Sitemap = [
    "","/workouts",...WORKOUT_TEMPLATE_CATEGORIES.map(category=>`/workouts/${category.slug}`),"/workout-programs","/privacy","/terms","/subscribe","/community",
  ].map(path=>({url:`${siteUrl}${path||"/"}`,lastModified:now,changeFrequency:path.startsWith("/workouts")||path==="/workout-programs"?"weekly":"monthly",priority:path===""?1:path==="/workouts"?0.9:path==="/workout-programs"?0.95:path.startsWith("/workouts/")?0.82:0.6}));
  ...
  return [...staticPages,...workoutPages,...programPages];
}
