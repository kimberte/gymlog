...
export default function TestHomepage() {
  ...
  return <main className="test-homepage">
    <div className="test-home-shell">
      ...
      <section className="test-home-actions" aria-label="Gym Log navigation">
        {actions.map(({ label, href, Icon, soon }) => {
          const content = <><span className="test-action-icon"><Icon size={29} /></span><span className="test-action-label">{label}</span>{soon && <span className="test-action-soon">Coming soon</span>}</>;
          if (label === "Settings") return <button key={label} type="button" className="test-action-card" onClick={handleSettings}>{content}</button>;
          if (soon) return <button key={label} type="button" className="test-action-card test-action-disabled" onClick={() => showToast("Blog is coming soon")}>{content}</button>;
          return <Link key={label} href={href!} className="test-action-card">{content}</Link>;
        })}
      </section>

      <Link href="/" className="test-today-card">
        <div className="test-today-top"><span className="test-section-kicker">TODAY</span><span className="test-open-link">Open calendar →</span></div>
        <div className="test-today-date">{formatToday()}</div>
        <div className="test-today-workout">{primaryWorkout}</div>
        {workoutCount > 1 && <div className="test-today-extra">+ {workoutCount - 1} more workout{workoutCount - 1 === 1 ? "" : "s"}</div>}
        {!workoutCount && <div className="test-today-extra">Tap to log today's training</div>}
      </Link>
    ...
    {toast && <div className="test-home-toast" role="status">{toast}</div>}
  </main>;
}
